...
Junto a PLATEAR, los planes especiales aportan reglas sectoriales directamente reutilizables. El Plan Especial de Protección Civil ante Fenómenos Meteorológicos Adversos en Aragón (PROCIFEMAR) \cite{PROCIFEMAR} justifica la inclusión del nivel de aviso AEMET y del fenómeno meteorológico activo. El Plan Especial de Protección Civil ante el Riesgo de Inundaciones en Aragón (PROCINAR) \cite{PROCINAR} respalda el uso de la peligrosidad por inundación como variable contextual. El Plan Especial de Protección Civil ante Emergencias por Accidentes en el Transporte de Mercancías Peligrosas por Carretera y Ferrocarril en Aragón (PROCIMER) \cite{PROCIMER} y el marco Seveso \cite{RD840_2015} justifican la consideración específica del riesgo químico e industrial. El Plan Especial de Protección Civil de Emergencias por Incendios Forestales en Aragón (PROCINFO) \cite{PROCINFO}, por su parte, aporta una lógica útil en relación con simultaneidad, progresión del incidente y amenaza a núcleos habitados o infraestructuras.
...
En este punto conviene separar dos planos. Las fuentes oficiales estatales y aragonesas justifican el marco normativo y operativo del sistema. El Registro de Emergencias del 112 de Castilla y León se incorpora únicamente como fuente empírica real para probar el pipeline de ingesta, limpieza, extracción de señales y trazabilidad. Su uso no modifica el caso piloto Aragón ni sustituye el marco normativo del sistema.
...
IDEARAGON & Contexto espacial y accesibilidad & Accesibilidad al punto, emplazamiento crítico & Requiere integración geoespacial \\ \hline
...
Registro de Emergencias del 112 de Castilla y León & Fuente empírica proxy para pipeline & Ingesta, limpieza, señales textuales, coordenadas y trazabilidad & No contiene prioridad P1--P4 ni sustituye al caso Aragón \\ \hline
\end{tabular}
...
El Sistema Nacional de Cartografía de Zonas Inundables (SNCZI) del Ministerio para la Transición Ecológica y el Reto Demográfico (MITECO) aporta una capa territorial oficial de gran relevancia para el prototipo. Su papel consiste en determinar si la localización del incidente cae dentro de zonas de peligrosidad por inundación oficialmente cartografiadas. Esta fuente es especialmente valiosa cuando se combina con el contexto meteorológico: un aviso localizado en una zona inundable durante un episodio de precipitación adversa tiene un potencial de deterioro superior al mismo aviso fuera de una zona de riesgo.

\subsection{INE: datos demográficos y vulnerabilidad}

Los datos demográficos del Instituto Nacional de Estadística (INE) se incorporan como apoyo a la estimación de exposición y vulnerabilidad territorial. Su función no es identificar afectados concretos, sino aportar un marco razonable de contexto cuando todavía no se dispone de información completa sobre las personas implicadas. Su uso debe ser prudente: el sistema no puede convertir estadísticas municipales en certezas individuales.

\subsection{IDEARAGON: cartografía oficial de Aragón}

La Infraestructura de Datos Espaciales de Aragón (IDEARAGON) aporta contexto territorial y de accesibilidad. Su interés para el sistema radica en que la prioridad operativa no depende solo del tipo de incidente o del riesgo directo, sino también de la facilidad o dificultad de llegar al punto, de la proximidad a equipamientos relevantes y de la localización dentro de un entorno territorial concreto.

\subsection{Registro Seveso}

El registro público de instalaciones sujetas a la normativa Seveso permite identificar entornos industriales que exigen una lectura reforzada del incidente. Su principal utilidad para el motor consiste en elevar la sensibilidad del sistema ante eventos que, por localización, puedan tener un potencial de escalado químico o industrial superior al que se observa en la descripción inicial. Esta fuente se emplea como apoyo contextual, no como indicador suficiente por sí solo para declarar una prioridad crítica.

\subsection{Memorias y estadísticas del 112 Aragón}

Las memorias y estadísticas del 112 Aragón son relevantes para entender el perfil operativo real del servicio y para evitar diseñar un sistema desconectado de su carga habitual. Su uso en el TFM es de calibración y plausibilidad, no de entrenamiento supervisado. A falta de microdatos operativos públicos de Aragón, estas memorias permiten contrastar que las familias de incidentes seleccionadas y el caso piloto territorial son razonables desde el punto de vista operativo.

\subsection{Registro de Emergencias del 112 de Castilla y León}

El Registro de Emergencias del 112 de Castilla y León \cite{Registro112CyL} se incorpora como fuente empírica real de incidentes procedentes de un servicio autonómico 112. Su función dentro del TFM es probar el pipeline de ingesta, limpieza, normalización textual, extracción de señales, geolocalización y trazabilidad sobre registros reales.

Este dataset no modifica el marco normativo del trabajo ni desplaza el caso piloto Aragón. Tampoco se utiliza como base de entrenamiento supervisado de prioridad, ya que no contiene una etiqueta oficial P1--P4 ni una escala equivalente. Su valor reside en aportar realismo textual y estructural al tratamiento de datos, mientras que la interpretación operativa de la prioridad se mantiene vinculada al marco estatal y aragonés.
...
La cuarta es que las fuentes oficiales disponibles permiten enriquecer el contexto del incidente, pero no eliminan las limitaciones de acceso a datos operativos reales del 112 Aragón. Por tanto, el TFM debe mantener un enfoque realista: baseline interpretable, variables justificadas, dataset piloto híbrido y validación humana obligatoria. El Registro de Emergencias del 112 de Castilla y León se emplea como fuente empírica proxy para el pipeline, no como sustituto normativo ni territorial del caso Aragón.

La quinta es que la escala P1--P4 del sistema debe seguir tratándose como una abstracción operativa propia del prototipo, cuya inspiración en las situaciones operativas 0--3 es metodológicamente adecuada, pero que no debe presentarse como equivalencia jurídica.
