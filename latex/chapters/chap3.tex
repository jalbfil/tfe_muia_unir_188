...
En v0.1.0, la taxonomía se deriva de dos fuentes: el catálogo de riesgos y categorías operativas recogido en la normativa, y los campos textuales del Registro 112 CyL. El campo estructurado de tipo de incidente resulta útil como orientación inicial, pero el sistema no debe depender exclusivamente de él. La descripción textual y el título del incidente son procesados por la Capa 1 mediante un clasificador multietiqueta de aprendizaje automático (TF-IDF + Regresión Logística) acoplado a guardarraíles deterministas en un esquema neuro-simbólico, lo cual permite detectar señales críticas como \texttt{signal\_fallecido}, \texttt{signal\_herido\_grave}, \texttt{signal\_atrapado}, \texttt{signal\_intoxicacion}, \texttt{signal\_incendio}, \texttt{signal\_ accidente\_trafico}, \texttt{signal\_rescate} o \texttt{signal\_meteo\_inundacion} con precisión y latencia submilisegundo.
