...
El proyecto mantiene tambien un baseline experto y un XGBoost ceiling. El baseline experto permite disponer de una referencia transparente, construida a partir de conocimiento normativo y se\~nales operativas. XGBoost funciona como techo opaco no productivo: sirve para comparar rendimiento potencial, pero no sustituye al nucleo interpretable. Esta separacion evita presentar un modelo de caja negra como decision operativa en un dominio de alto impacto.

La literatura sobre interpretabilidad refuerza esta decision. Rudin \cite{Rudin2019} defiende que, en decisiones de alto riesgo, debe preferirse un modelo interpretable desde el dise\~no cuando sea viable, en lugar de explicar a posteriori una caja negra. Esta idea es coherente con la arquitectura del TFM: RuleFit decide la prioridad; el LLM explica y comunica, pero no decide de forma autonoma.

\section{Explicabilidad, LLM local, RAG y MCP}

La explicabilidad en este TFM no se reduce a mostrar una probabilidad. La recomendacion debe acompa\~narse de reglas activadas, confianza, advertencias cuando proceda y citas normativas. Para ello, la Capa 3 utiliza un LLM local cuantizado, recuperacion aumentada sobre un corpus normativo de Castilla y Leon y herramientas MCP.

El uso de un LLM local responde a dos necesidades: generar explicaciones comprensibles para el operador y preservar soberania del dato. La recuperacion aumentada (RAG) evita que la explicacion dependa solo de memoria generativa: el sistema consulta fragmentos del corpus normativo y los utiliza para sustentar citas legales. MCP aporta un mecanismo estandar para exponer herramientas como busqueda normativa, detalle de reglas y cita legal de prioridades; Anthropic lo presento en 2024 como un protocolo abierto para conectar asistentes de IA con fuentes de datos y herramientas externas \cite{Anthropic2024MCP}.

Esta capa debe entenderse con precision metodologica. El LLM no decide la prioridad ni debe inventar fundamento juridico. Su funcion es traducir una recomendacion ya calculada en una explicacion breve, trazable y revisable, con modo degradado si el modelo no esta disponible o supera el SLA.

\section{Sistemas y referencias relacionadas}

Existen varias familias de trabajos y sistemas relacionados. AIDR demostro la utilidad de combinar aprendizaje automatico y participacion humana para clasificar mensajes de crisis en redes sociales \cite{Imran2014AIDR}. TREC Incident Streams proporciono benchmarks para clasificar mensajes de crisis y estimar criticidad \cite{TRECIS2021}. GDACS integra alertas globales multirriesgo e informacion para coordinacion internacional \cite{GDACS2025}. Copernicus Emergency Management Service aporta cartografia rapida y productos geoespaciales para apoyar la respuesta \cite{CopernicusEMS2025}. Las revisiones recientes del JRC y SAPEA subrayan el potencial de la IA en gestion de emergencias, pero tambien la necesidad de control humano, interpretabilidad y robustez \cite{JRC2025AI,SAPEA2025}.
